...
class MapNotifier<K, V> extends ValueNotifier<Map<K, V>> implements Map<K, V> {
  /// Creates a notifier backed by an initial map.
  MapNotifier(Map<K, V> initial) : super(Map<K, V>.from(initial));
  ...
  void replace(Map<K, V> newValue) {
    ...
    value = Map<K, V>.from(newValue);
  }

  /// Clones the current map so listeners are notified after in-place changes.
  void refresh() {
    final map = value;
    final newMap = {...map};
    value = newMap;
  ...
  /// Executes [action] against the current map and refreshes afterward.
  ///
  /// This acts as an explicit notification boundary for in-place or nested
  /// mutations that do not replace the map instance.
  R mutate<R>(R Function(Map<K, V> map) action) {
  ...
  void operator []=(K key, V value) =>
      _updateOnAction(() => this.value[key] = value);
  ...
  bool isEqual(Map<K, V> other) => mapEquals(value, other);
}
