...
class ListenablesBuilder extends StatefulWidget {
  /// Creates a widget that rebuilds when any item in [listenables] notifies.
...
class _ListenablesBuilderState extends State<ListenablesBuilder> {
  ...
  void _listener() {
    ...
    if (threshold == null ||
        _lastBuildTime == null ||
        now.difference(_lastBuildTime!) >= threshold) {
    ...
    final remaining = threshold - now.difference(_lastBuildTime!);
